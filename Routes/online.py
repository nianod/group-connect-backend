...
from Database.Users.db import online_collection
...
def create_online_session(
    ...
    result = online_collection.insert_one(online_data)
    new_online = online_collection.find_one({"_id": result.inserted_id})
...
@router.get("/")
def get_all_online():
    online = [online_helper(o) for o in online_collection.find()]
    return {"online": online}


@router.get("/my")
def get_my_online(current_user=Depends(get_current_user)):
    online = list(online_collection.find({"creatorId": str(current_user["_id"])}))
    for o in online:
        o["_id"] = str(o["_id"])
    return online
